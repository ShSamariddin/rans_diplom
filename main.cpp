...
void read(int st_id, int end_id, uint8_t *data) {
    PGM pic[3];
    int cnt = 1;
...
int main(int argc, char *argv[]) {
    ...
    int pgm_num = (last_file - first_file) / step;
...
#pragma omp parallel num_threads(core_num)
    {
        ...
        if (thread_id == core_num - 1) {
            read(thread_id * pgm_per_thread + 1, pgm_num, data);
        } else {
            read(thread_id * pgm_per_thread + 1, (thread_id + 1) * pgm_per_thread, data);
        }
    }
...
#pragma omp parallel num_threads(core_num)
    ...
    std::cout << total_res << '\n';
    s_end = time(NULL);
    printf("rANS: %f bytes\n", 100.0 - (total_res * 100.0) / (in_size * 1.0));
    std::cout << "\ntime finnal : " << s_end - s_begin << '\n';
}
